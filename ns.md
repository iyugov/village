...
1. На сервере "NSs" откроем файл настроек сервера SSH:
...
1. На сервере "NS" установим *fail2ban*:
...
1. На сервере "NS" установим пакеты `postfix` и `mailutils`:
...
2. В разделе `nameservers` для имеющегося сетевого интерфейса изменим адрес сервера имён на 10.0.1.3, также укажем домен поиска "village":
...
2. В разделе `nameservers` для имеющегося сетевого интерфейса изменим адрес сервера имён на 10.0.1.3 (собственный адрес) и добавим домен поиска "village":
